...
def create_unrequested_list(outputfile="notRequestestedSIS.txt", term="2020A"):
    ...
    print(
        "-> NEXT: Please now run `create_unused_sis_list(inputfile='%s')` to determine which of these course codes are already used in Canvas"
        % outputfile
    )
...
def create_unused_sis_list(
    ...
    print(
        "-> NEXT: To create Requests for these courses please run `create_requests(inputfile='%s')`"
        % outputfile
    )
